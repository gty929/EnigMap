...
typedef uint64_t K;
typedef uint64_t V;
constexpr inline K INVALID_KEY = DUMMY<K>();
constexpr inline V INVALID_VALUE = DUMMY<V>();
...
struct Node {
  ...
  auto operator==(const Node& other) const {
    return true * (k == other.k) * (v == other.v) *
  ...
  friend std::ostream& operator<<(std::ostream& o, const Node& x) {
    // UNDONE(): c++14 libraries with c++20 compiler don't have << working for
    // non string types.
    //
    // o << "(" << std::to_string(x.k) << ", " << std::to_string(x.v) << ", "
    //   << x.child[0] << ", " << x.child[1] << "," <<
    //   Dir_t_to_string(x.balance)
    //   << ")";
    return o;
  }
#endif

  static consteval INLINE Node DUMMY() {
    return Node{K{INVALID_KEY},
                V{INVALID_VALUE},
...
template <>
INLINE void CMOV<_OBST::Node>(const uint64_t& condition, _OBST::Node& A,
                              const _OBST::Node& B) {
