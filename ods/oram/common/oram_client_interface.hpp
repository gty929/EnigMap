...
// eb-indexes in large buckets, and forward calls the storage class fileServer
// with the eb index.
//
...
template <typename Block = _ORAM::Block::Block<>,
          ...
          unsigned int DIRECTLY_CACHED_LEVELS =
              ORAM_SERVER__DIRECTLY_CACHED_LEVELS>
  requires ::Concepts::Encryptable<
...
struct ORAMClientInterface {
  ...
  // using LargeBucket_LastLevel_t = LargeBucket::LargeBucket<typename
  // Bucket::Encrypted_t,ENCRYPT_LARGE_BUCKETS,1>; UNDONE(): paramterize
  // ORAM_SERVER__BUCKET_CACHE_SIZE here
  Cache<LargeBucket_t, ORAM_SERVER__BUCKET_CACHE_SIZE> cache;

  static_assert(DIRECTLY_CACHED_LEVELS % LEVELS_PER_PACK == 0);
  static constexpr uint64_t DC_LARGE_BUCKET_LEVELS =
      DIRECTLY_CACHED_LEVELS / LEVELS_PER_PACK;
  ...
  EM::MemoryServer::NonCachedServerFrontendInstance<
      LargeBucket_t, ::EM::Backend::MemServerBackend, true, false, false>
      server;

  OCI_ONLY(ORAMInterfaceVerifier<Block, Bucket> verifier;)

  explicit ORAMClientInterface(uint64_t N)
      : N_(N),
        L_(CeilLog2(N)),
        V_(1ULL << ((L_ + 1 + LEVELS_PER_PACK - 1) / LEVELS_PER_PACK *
                    LEVELS_PER_PACK)),
    ...
    // UNDONE(ttt):
    // data.resize(1 << (DIRECTLY_CACHED_LEVELS/LEVELS_PER_PACK),
    // LargeBucket_t());
  }
  ...
  void ReadBucketMetadata(const Index& pos, const Index& depth,
                          BucketMetadata_t& ret) {
  ...
  void WriteBucketMetadata(const Index& pos, const Index& depth,
                           const BucketMetadata_t& val) {
  ...
  void ReadBlock(const Index& pos, const Index& depth, const Index& offset,
                 Block_t& ret) {
  ...
  void WriteBlock(const Index& pos, const Index& depth, const Index& offset,
                  const Block_t& val) {
    Assert(offset < Bucket::BUCKET_SIZE);
    Bucket& bucket = GetBucketRef(pos, depth);
